class BudgetIncome < ActiveRecord::Base
  ...
  def generate_income_splits(first_split_date, initial_income)
    ...
    if self.income.weekly
       calculate_weeks(first_split_date,initial_income,split_income,total_monthly_income)
  ...
  def calculate_weeks(start_date,starting_salary,weekly_income,total_monthly_income)
    ...
    while month_complete == false do
        if first_week
          ...
          # weeks << Week.new(current_week,weekly_income,total_salary,next_month)    
           income_splits << IncomeSplit.new(budget_id: self.budget.id, income_id: self.income.id, income_split_date: current_week,
              amount: weekly_income.to_f,total_received: total_salary, is_last_for_month: false)
        end   
        if total_salary > total_monthly_income.to_f
          next_month = total_salary - total_monthly_income
          total_salary -= next_month
          remaining_for_month = weekly_income - next_month
